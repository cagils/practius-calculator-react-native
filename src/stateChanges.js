...
export const updateStateOnCarry = (prevState, props) => {
    return ({parenCount: 0, input: prevState.result})
}

export const updateStateOnDel = (prevState, props) => {
    let par = prevState.parenCount
    let lastChar = prevState.input.slice(-1)
...
export const updateStateOnEqu = (prevState, props) => {
    ...
    try {
        ...
        let type = 'error'
        if (e.message.startsWith('Parenthesis') ||
            e.message.startsWith('Unexpected end')) {
            type = 'warning'
        }
        return ({error: {type: type, name: e.name, message: e.message}})
    }
...
export const updateStateOnPaste = (clipboard) => (prevState, props) => ({input: prevState.input + clipboard})

export const updateStateOnDefault = (id) => (prevState, props) => ({input: prevState.input + buttons[id].value})
